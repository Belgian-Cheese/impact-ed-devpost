document.addEventListener('DOMContentLoaded', () => {
    ...
    function redirectToIndexIfMobileOrTablet() {
        const isMobile = /Android|webOS|iPhone|iPad|IPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        const isTablet = /(tablet|ipad|playbook|silk)|(android(?!.*mobi))/i.test(navigator.userAgent);
        const isSmallScreen = window.innerWidth < 600;

        if (isMobile || isTablet || isSmallScreen) {
          sessionStorage.setItem('unsupportedRedirect', 'true');
          let redirectURL = 'index.html?fromParaPage=true&unsupported=true';
          window.location.href = redirectURL;
        }
    }

    redirectToIndexIfMobileOrTablet();
        ...
        db.collection('users').doc(user.email).get()
            .then((doc) => {
                ...
                if (doc.exists) {
                    ...
                    // --- Strict Comparison ---
                    const requiredType = "For People Without Motor Abilities";
...
// The entire logic is now handled within the onAuthStateChanged listener callback.
    ...
    // DO NOT call checkUserTypeAndRedirect() directly anymore.
    // The onAuthStateChanged listener handles the logic now.
    ...
    let recognition;
    let currentLanguage = "en"; // Default language code 'en', 'es', 'hi'
    let speechLangCode = "en-US"; // Default for Speech API
    let newLanguage = "en"; // Track changes
    let lastProcessedTranscript = "";

    const SOS_TRIGGER_WORDS = ["help", "assist", "ayuda", "bachao", "sos", "emergency"]; // Keep SOS words simple and multi-lingual if possible

    // This function is now responsible ONLY for requesting permissions.
    async function requestMediaPermissions() {
        try {
            ...
            // If there is stream, the microphone access is allowed.
            if (stream) {
                stream.getTracks().forEach(track => track.stop());
                console.log("Stream stopped for permissions check.");
                return true; // Permission was granted
            }

            return true; // Handle the edge-case, permission was granted but stream didn't start

        } catch (error) {
            console.error("Microphone permission DENIED:", error);
            alert(i18n[currentLanguage]?.['micPermissionError'] ||
                "Microphone access is needed for voice commands. Please grant permission in your browser settings."); // Use currentLanguage here
    ...
    async function setupSpeechRecognition() {
        try {
            if (!SpeechRecognition) {
                ...
                alert("Speech Recognition API is not supported in your browser. Please try a different browser");
                return;
            }

            const hasPermissions = await checkAndRequestPermissions();  // call the new function here

            if (hasPermissions) { // Only proceed if permissions are granted
                console.log("Proceeding with Speech Recognition setup...");
                startRecognition();
                return;
            } else {
                console.warn("Speech Recognition setup aborted: Microphone permission denied.");
                return;
            }
    ...
    async function checkAndRequestPermissions() {
       if (!mediaPermissionsRequested) {
           ...
           console.log("Media permissions already requested.");
           return true; // Assume permissions were previously granted if already requested.
       }
   }
    ...
    function startRecognition() {
        ...
        console.log("Speech Recognition starting new flow...");

        speechLangCode = "en-US";
        console.log("Setting up speech recognition for language:", speechLangCode);
        ...
        recognition.onresult = (event) => {
            ...
            if (transcript && transcript !== lastProcessedTranscript) {
                ...
                lastProcessedTranscript = transcript; // Update last processed
            }
        };

        recognition.onerror = (event) => {
            console.error("Speech recognition error:", event.error);
            // Restart when there is `no-speech` error
            if (event.error === 'no-speech') {
                console.warn("No speech detected, restarting recognition.");
                restartRecognition();
            } else {
                // Restart when other errors occur as well.
                restartRecognition();
            }
        };
    ...
    function restartRecognition() {
        if (recognition) {
            try {
                ...
                console.warn("Error stopping recognition before restart:", e);
            }
            recognition = null;
            setupSpeechRecognition();
        } else {
            // If recognition object is lost, try setting it up again
            console.log("Recognition object lost, attempting full setup again.");
            fetchAndSetupSpeech();
        }
    }
    ...
    function handleSpeech(transcript) {
        ...
        // 1. Check for SOS trigger first (most critical)
        if (SOS_TRIGGER_WORDS.some((word) => transcript.includes(word))) {
            handleSosTrigger(transcript);
            return;
        }

        console.log("No SOS trigger detected in:", transcript);
    }

    async function handleSosTrigger(transcript) {
        console.log("SOS Triggered by Voice:", transcript);
        const sosButton = document.getElementById('sosButton');
        if (sosButton && !sosButton.disabled) {
            sosButton.click();
        } else {
            console.warn("SOS Button not found or already active.");
        }
        lastProcessedTranscript = "";
    }

    async function fetchAndSetupSpeech() {
        try {
            // Ensure permissions are requested/checked before setup
    ...
    //call fetchAndSetupSpeech() in the DOMContentLoaded
    fetchAndSetupSpeech();
    ...
    menuItems.forEach((item, index) => {
        item.addEventListener("click", () => {
            ...
            // Scroll entire content area to top
            const content = document.querySelector('.content');
            if (content) {
                content.scrollTo({ top: 0, behavior: 'smooth' });
            }
    
            // Also scroll the window to top (optional, in case some content goes over)
    ...
    function disableDarkMode() {
        ...
        document.querySelectorAll('.circle-progress').forEach(el => el.classList.remove('dark-mode'));
         ...
         document.querySelectorAll('#study .chapter-content-container').forEach(el => el.classList.remove('dark-mode'));
          ...
          document.querySelectorAll('.ai-message').forEach(el => el.classList.remove('dark-mode'));
          document.querySelectorAll('#study h2').forEach(el => el.classList.remove('dark-mode'));
  ...
  // Add to the main auth check (replace existing code):
  (async function() {
...
function displayChapters(subject) {
  ...
  window.convertToAudio = async function () {
    ...
    try {
        ...
        // Store audio object in window object
        ...
        playButton.textContent = 'Play'; // Ensure it says Play initially
        handleAudioConversionSuccess(); // Change convert button text and disable it
         ...
         // --- End ABORT CONTROLLER LOGIC ---
             ...
             convertButton.textContent = 'Conversion Failed';
             setTimeout(() => {
...
window.playAudio = function() {
    ...
    if (window.audio) { // Correctly access the global audio
        if (window.audio.paused) {
            window.audio.play();
            playButton.textContent = 'Pause'; // Change to Pause
        } else {
            window.audio.pause();
            playButton.textContent = 'Play'; // Change to Play
        }
    }
};
...
//  } -- REMOVE THIS LINE
 ...
 function displayChapterView(chapterName, chapterContent, subjectName) {
  ...
  // End any previously running chapter timer
  if (currentChapterSubjectId) {
      endChapterTimer(currentChapterSubjectId);  // VERY IMPORTANT: Stop previous timer.
  }
