...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkUserType() {
            ...
            .then(data => {
                ...
                if (data.userType !== 'For People Without Motor Abilities') {
    ...
    document.getElementById('placementTestForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            userData.hasTakenTest = true;
            userData.testCategory = determinedCategory;
    ...
    async function syncTotalTimeWithServer() {
        if (!currentUserEmail || sessionStartTime === null) return; // Ensure session has started
        ...
        try {
            await fetch(`${API_BASE_URL}/updateTotalTime`, { // NEW SERVER ENDPOINT
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: currentUserEmail, totalTimeSpent: totalTimeSpent })
            });
            console.log("Total time synced with server.");
            updateTotalTimeDisplay(); // Update UI
        } catch (error) {
            console.error("Error syncing total time with server:", error);
        }
    }
